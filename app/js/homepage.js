...
function setupFadeInAnimations() {
    const animatedTags = selectAll('h1, h2, h3, p, .about__img, a.button, .footer__img, .topnav__homelink, .bg-text-hero, .topnav__item, .footer__logo, .footer__links, .subfeature__img, .subfeature__accent, .subfeature__feature-img');
...
function setupAccentAnimations() {
    ...
    debug(`Found ${breathers.length} breather elements and ${spinners.length} spinner elements`);
...
function setupParallaxEffects() {
    ...
    debug(`Hero section found: ${!!heroSection}, Features container found: ${!!featuresContainer}, Number of sections: ${sections.length}`);
    ...
    function applyFeaturesParallax() {
        ...
        sections.forEach((section, index) => {
            ...
            debug(`Applying parallax to section ${index + 1}, ${parallaxTags.length} parallax elements`);

            parallaxTags.forEach((tag) => {
                const speed = parseFloat(tag.getAttribute('data-parallax'));
                tag.style.transform = `translate3d(0, ${distanceToSection * speed}px, 0)`;
            });

            // Background color change logic
            const dataBackground = section.getAttribute('data-background');
            if (dataBackground) {
                const sectionVisibility = (scrollY + viewportHeight / 2 - topSection) / section.offsetHeight;
                if (sectionVisibility > 0 && sectionVisibility < 1) {
                    let backgroundColor = dataBackground;
                    if (backgroundColor.startsWith('var(')) {
                        backgroundColor = getComputedStyle(document.documentElement).getPropertyValue(backgroundColor.slice(4, -1).trim());
                    }
                    featuresContainer.style.backgroundColor = backgroundColor;
                    debug(`Changed background color to ${backgroundColor} for section ${index + 1}`);
                }
            }
        });
    }
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeAnimationsAndParallax);
} else {
